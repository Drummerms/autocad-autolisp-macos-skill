# To Specify Whether a Block Reference Can Be Exploded

1. Click
   Drafting tab ![](../images/ac.menuaro.gif) Block panel ![](../images/ac.menuaro.gif) Block Editor.
   ![](../images/GUID-E3DDBE51-C92A-4D07-A51D-C7C685E4F578-low.png)
2. Open the block definition in the Block Editor.
3. In the Block Editor, make sure nothing is selected.
4. In the Properties Inspector, under Block, click Allow Exploding.
5. On the drop-down list, select Yes or No.
6. In the Block Editor, make changes to the block and save. At the Command prompt, enter BSAVE.
7. (Optional) If you are finished using the Block Editor, click Close Block Editor.

#### Related References

* [Commands for Basic Blocks](Commands-for-Basic-Blocks.md)

#### Related Concepts

* [About Modifying Block Definitions](About-Modifying-Block-Definitions.md)

#### Related Information

* [To Modify the Description of a Block](To-Modify-the-Description-of-a-Block.md)
* [To Ensure Uniform Scaling in a Block Reference](To-Ensure-Uniform-Scaling-in-a-Block-Reference.md)
* [To View Properties of a Block Definition (Block Editor)](To-View-Properties-of-a-Block-Definition-Block-Editor.md)
* [About Modifying Block References](About-Modifying-Block-References.md)
* [To Explode a Block Reference](To-Explode-a-Block-Reference.md)

[Please send us your comment about this page](..)

[![Creative Commons License](../images/ccLink.png)](http://creativecommons.org/licenses/by-nc-sa/3.0/) *Except where otherwise noted, this work is licensed under a [Creative Commons Attribution-NonCommercial-ShareAlike 3.0 Unported License](http://creativecommons.org/licenses/by-nc-sa/3.0/). Please see the [Autodesk Creative Commons FAQ](http://autodesk.com/creativecommons) for more information.*