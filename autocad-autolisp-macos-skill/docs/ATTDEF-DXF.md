...
The following group codes apply to attdef (attribute definition) entities. In addition to the group codes described here,
see Common Group Codes for Entities.
...
If group 72 and/or 74 values are nonzero, then the first alignment point values are ignored and new values are calculated
by
AutoCAD based on the second alignment point and the length and height of the text string itself (after applying the text style).
If the 72 and 74 values are zero or missing, then the second alignment point is meaningless.
