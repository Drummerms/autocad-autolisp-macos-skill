...
ERRNO system variable is set to one of these values when an AutoLISP function call causes an error that AutoCAD detects. AutoLISP
applications can inspect the current value of
ERRNO with
(getvar "errno").

The
ERRNO system variable is not always cleared to zero. Unless it is inspected immediately after an AutoLISP function has reported
an error, the error that its value indicates may be misleading. This variable is always cleared when starting or opening a
drawing.

NOTE:The possible values of
ERRNO, and their meanings, are subject to change. When an error occurs, not all functions return an error code value greater than
0. For example, the following statements return an error code value of 0 but you can capture and identify the error by utilizing
the
