# To Add, Remove, or Disable a Breakpoint while Debugging an LSP File (AutoLISP/VS Code)

NOTE:Debugging is not available in AutoCAD LT.

1. In Visual Studio Code, open and activate an editor window that contains the AutoLISP source code to debug.
2. In the margin of the active editor window, do one of the following:
   * *Add a breakpoint* – Click to the left of a line
   * *Remove a breakpoint* – Click an existing breakpoint
   * *Disable a breakpoint* – Right-click (or secondary click on Mac OS) an existing breakpoint and choose Disable Breakpoint

   ![](../images/GUID-0448E8A4-6772-41ED-BC01-0A79F50FC3D8-low.png)

   TIP:You can also use the Breakpoints section on the Debug and Run view to manage active and disabled breakpoints while debugging.

   ![](../images/GUID-6EA7FA84-61C6-48C2-A3A5-5B80B0272BFA-low.png)
3. Click Start Debugging or press F5 to load and debug the AutoLISP source code in the active window.

#### Related Information

* [Configuring VS Code (AutoLISP/VS Code)](Configuring-VS-Code-AutoLISP-VS-Code.md)
* [To Launch and Attach an Instance of AutoCAD to VS Code for Debugging (AutoLISP/VS Code)](To-Launch-and-Attach-an-Instance-of-AutoCAD-to-VS-Code-for-Debugging-AutoLISP-VS-Code.md)
* [To Watch Variables and Expressions while Debugging an LSP File (AutoLISP/VS Code)](To-Watch-Variables-and-Expressions-while-Debugging-an-LSP-File-AutoLISP-VS-Code.md)
* [To Access the Call Stack while Debugging an LSP File (AutoLISP/VS Code)](To-Access-the-Call-Stack-while-Debugging-an-LSP-File-AutoLISP-VS-Code.md)

[Please send us your comment about this page](..)

[![Creative Commons License](../images/ccLink.png)](http://creativecommons.org/licenses/by-nc-sa/3.0/) *Except where otherwise noted, this work is licensed under a [Creative Commons Attribution-NonCommercial-ShareAlike 3.0 Unported License](http://creativecommons.org/licenses/by-nc-sa/3.0/). Please see the [Autodesk Creative Commons FAQ](http://autodesk.com/creativecommons) for more information.*